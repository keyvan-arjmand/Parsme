...
public class PaymentController : Controller
{
    ...
    // GET
    public PaymentController(IMediator mediator, ILogger<HomeController> logger, IUnitOfWork work, RoleManager<Role> roleManager, UserManager<User> userManager, SignInManager<User> signInManager)
    {
        _mediator = mediator;
        _logger = logger;
        _work = work;
        ...
        int userAddressId,string desc)
    {
        if (User.Identity.IsAuthenticated)
        {
            ...
            foreach (var i in    JsonConvert.DeserializeObject<List<int>>(HttpContext.Session.GetString("basket")).ToList())
            {
