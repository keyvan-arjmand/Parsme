...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
            ...
            await _responseCacheService.GetOrSetCacheAsync<ProductRedis>(
                ...
                async () =>
                {
                    var categoriesFromDb = await _work.GenericRepository<Product>().TableNoTracking
                        ...
                        .AsSplitQuery()
                        .Where(x => x.IsShowIndex && x.ProductStatus == ProductStatus.Available &&
                                    x.ProductColors.Any(x => x.Inventory > 0))
                        .Take(20).ToListAsync();
        ...
        foreach (var i in CatsViews.Select(x => x.Id).ToList())
        {
            otherProd.AddRange(await _work.GenericRepository<Product>().TableNoTracking
                ...
                .Include(x => x.Offer)
                .Include(x => x.ProductColors).ThenInclude(x => x.Color)
                .Where(x => x.ProductStatus == ProductStatus.Available &&
                            x.ProductColors.Any(x => x.Inventory > 0))
                .AsSplitQuery()
                .Take(4).ToListAsync());
        }
            ...
            await _responseCacheService.GetOrSetCacheAsync<ProductRedis>(
                ...
                async () =>
                {
                    var categoriesFromDb = await _work.GenericRepository<Product>().TableNoTracking
                        ...
                        .Include(x => x.ProductColors).ThenInclude(x => x.Color)
                        .AsSplitQuery()
                        .Where(x => x.MomentaryOffer && x.ProductStatus == ProductStatus.Available &&
                                    x.ProductColors.Any(x => x.Inventory > 0)).Take(7).ToListAsync();
                    return _mapper.Map<List<ProductRedis>>(categoriesFromDb);
                } // مدت زمان انقضا کش
            ));
    ...
    public async Task<IActionResult> CanceledFactor(int id)
    {
        ...
        try
        {
            ...
            var result = webApi.VerifyLookup(factor.User.PhoneNumber, factor.User.Name.Replace(" ", "-"),
                factor.FactorCode,
                string.Empty,
                "UserCanceledFactor");
        }
    ...
    public async Task<IActionResult> Search(string search, double min, double max, int page = 1)
    {
        ...
        if (max > 0)
        {
            ...
            ViewBag.Products = await query.OrderByDescending(x =>
                    x.ProductStatus == ProductStatus.Available &&
